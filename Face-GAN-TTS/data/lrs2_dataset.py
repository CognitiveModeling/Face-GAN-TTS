...
class lrs2Dataset(torch.utils.data.Dataset):
    def __init__(self, split: str = "", config=None):
        ...
        if self.split == "train":
            self.filelist = self.config["lrs2_train"]
            self.video_dir = os.path.join(self.config["lrs2_path"], "trainval")
            self.audio_dir = os.path.join(self.config["lrs2_path"], "wav/trainval")
        elif self.split == "val":
            self.filelist = self.config["lrs2_val"]
            self.video_dir = os.path.join(self.config["lrs2_path"], "trainval")
            self.audio_dir = os.path.join(self.config["lrs2_path"], "wav/trainval")
        elif self.split == "test":
            self.filelist = self.config["lrs2_test"]
            self.video_dir = os.path.join(self.config["lrs2_path"], "test")
            self.audio_dir = os.path.join(self.config["lrs2_path"], "wav/test")
    ...
    def __getitem__(self, index):
        ...
        aud_denoised = nr.reduce_noise(
                y=aud_np,
                sr=sr,
                stationary=True,
                prop_decrease=self.config["denoise_factor"],
